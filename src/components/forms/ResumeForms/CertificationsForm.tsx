    ...
    <div className="space-y-6">
      ...
      <Card className="p-6 border-dashed border-2 border-border/50 bg-muted/20">
        <div className="space-y-4">
          ...
          <Button
            ...
            disabled={!newCertification.name || !newCertification.issuer || !newCertification.dateEarned}
          >
      ...
      <div className="space-y-2">
        {certifications.map((cert) => (
          <Card key={cert.id} className="p-4 flex justify-between items-start border-dashed border-2 border-border/50">
            <div className="space-y-1">
              <h4 className="font-medium">{cert.name}</h4>
              <p className="text-sm text-muted-foreground">{cert.issuer}</p>
              {cert.dateEarned && <p className="text-xs text-muted-foreground">Earned: {cert.dateEarned}</p>}
              {cert.expirationDate && <p className="text-xs text-muted-foreground">Expires: {cert.expirationDate}</p>}
              {cert.credentialId && <p className="text-xs text-muted-foreground">Credential ID: {cert.credentialId}</p>}
              {cert.url && (
