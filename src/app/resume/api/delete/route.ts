  ...
  try {
    ...
    if (result.deletedCount === 0) {
      await logAdminActivity(
        `Failed delete attempt - resume not found: ${resumeId}`,
        "warning",
        "resume",
        userEmail,
        { resumeId }
    ...
    await logAdminActivity(`Failed to delete resume: ${(error as Error).message}`, "error", "resume", null);
    return NextResponse.json(
      { success: false, message: "Server error while deleting resume" },
      { status: 500 }
    );
